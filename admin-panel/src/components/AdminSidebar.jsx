...
const AdminSidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    { path: '/rentals', label: 'Rentals', icon: 'building' },
    { path: '/rides', label: 'Rides', icon: 'transport' },
    { path: '/reports', label: 'Reports', icon: 'help' },
    { path: '/broadcast', label: 'Broadcast', icon: 'megaphone' },
    { path: '/monitoring', label: 'Monitoring', icon: 'chart' },
      ...
      <aside className={`fixed left-0 top-0 h-full w-64 bg-white border-r border-neutral-200 z-50 shadow-sm transition-transform duration-300 transform lg:translate-x-0 ${
        ...
        <div className="p-6 border-b border-neutral-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-neutral-900">AdminPanel</span>
          </div>
          {/* Mobile Close Button */}
          <button
            onClick={onClose}
        ...
        <div className="p-4 border-b border-neutral-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-neutral-900">{user?.name || 'Administrator'}</p>
              <p className="text-sm text-neutral-500 capitalize">Platform Admin</p>
            </div>
          </div>
        </div>
